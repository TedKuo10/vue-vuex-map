            ...
            <table>
              ...
              <tbody>
                <tr>
                  <th>早上</th>
                  <!-- <td></td> -->
                  <td v-for="(item, index) in servicePeriods[0]" :key="index">{{item}}</td>
                </tr>
                <tr>
                  <th>中午</th>
                  <!-- <td></td> -->
                  <td v-for="(item, index) in servicePeriods[1]" :key="index">{{item}}</td>
                </tr>
                <tr>
                  <th>晚上</th>
                  <!-- <td></td> -->
                  <td v-for="(item, index) in servicePeriods[2]" :key="index">{{item}}</td>
                </tr>
              </tbody>
            </table>

            <h2 class="title">地址：
              <a :href="`https://www.google.com.tw/maps/place/${currStore.address}`">{{ currStore.address }}</a>
            </h2>
  ...
  computed: {
    ...
    currStore () {
      return this.$store.state.stores.filter((item) => {
        // console.log(this.$store.state.infoBoxSid)
        return item.id === this.$store.state.infoBoxSid
      })[0]
    },
    servicePeriods () {
